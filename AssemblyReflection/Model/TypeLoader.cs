...
namespace AssemblyReflection.Model
{
    ...
    {
        internal static TypeMetadata LoadTypeMetadata(Type type, Dictionary<string, BaseMetadata> metaDictionary)
        {
            if (!metaDictionary.ContainsKey(type.FullName))
            {
                ...
                metaDictionary.Add(type.FullName, typeMetadata);

                typeMetadata.DeclaringType = EmitDeclaringType(type.DeclaringType);
                typeMetadata.Constructors = MethodLoader.EmitMethods(type.GetConstructors(), metaDictionary);
                typeMetadata.Methods = MethodLoader.EmitMethods(type.GetMethods(), metaDictionary);
                typeMetadata.NestedTypes = EmitNestedTypes(type.GetNestedTypes(), metaDictionary);
            ...
            {
                return metaDictionary[type.FullName] as TypeMetadata;
            }
        }
        ...
        private static IEnumerable<TypeMetadata> EmitNestedTypes(IEnumerable<Type> nestedTypes, Dictionary<string, BaseMetadata> metaDictionary)
        {
            return from type in nestedTypes
                   where type.IsVisible()
                   select LoadTypeMetadata(type, metaDictionary);
        }
